...
export function CartDrawer() {
  const { isCartOpen, setIsCartOpen, cartProducts, subtotal, setQty, removeFromCart, cartCount } = useShop();
  ...
  return (
    <Sheet open={isCartOpen} onOpenChange={setIsCartOpen}>
      <SheetContent side="right" className="flex w-[95vw] flex-col overflow-hidden p-0 sm:max-w-md bg-white">
        <SheetHeader className="border-b px-6 py-4 flex flex-row items-center justify-between">
          <SheetTitle className="font-display text-xl flex items-center gap-2 m-0">
            <span role="img" aria-label="cart text-xl">🛍</span> Your Cart ({cartCount})
          </SheetTitle>
        </SheetHeader>
          ...
          <>
              ...
              <ul className="space-y-6">
                {cartProducts.map(({ product, qty, size }) => (
                  <li key={product.id + (size || "")} className="flex gap-4">
                    <Link to={`/product/${product.slug}`} onClick={() => setIsCartOpen(false)} className="shrink-0">
                      <img
                    ...
                    <div className="flex min-w-0 flex-1 flex-col">
                      <div className="flex justify-between gap-2">
                        <div>
                          <h3 className="font-display text-base leading-tight">
                            <Link to={`/product/${product.slug}`} onClick={() => setIsCartOpen(false)} className="link-underline">
                              {product.name}
                            </Link>
                          </h3>
                          <p className="eyebrow mt-1 text-xs">{product.subcategory} {size ? `• ${size}` : ""}</p>
                        </div>
                        <Button 
                          variant="ghost" 
                          size="icon" 
                      ...
                      <div className="mt-auto flex items-end justify-between pt-3">
                        <div className="flex items-center rounded-sm border">
                          <Button variant="ghost" size="icon" className="h-7 w-7" aria-label="Decrease" onClick={() => setQty(product.id, qty - 1, size)}>
                            <Minus className="h-3 w-3" />
                          </Button>
                          <span className="w-8 text-center text-xs font-medium">{qty}</span>
                          <Button variant="ghost" size="icon" className="h-7 w-7" aria-label="Increase" onClick={() => setQty(product.id, qty + 1, size)}>
                            <Plus className="h-3 w-3" />
                          </Button>
                        </div>
                        <div className="text-right">
                          <p className="text-sm font-medium">{formatINR(getProductPrice(product, size))}</p>
                        </div>
                      </div>
                    </div>
                  </li>
            ...
            <div className="border-t bg-gray-50/50 p-6">
              <dl className="space-y-2 text-sm text-muted-foreground">
                ...
                <div className="flex justify-between">
                  <dt>Delivery</dt>
                  <dd className="font-medium text-foreground">{delivery === 0 ? "Free" : formatINR(delivery)}</dd>
                </div>
